...
function Prepend-PlainNumberText([xml]$OwnerDoc, [System.Xml.XmlNode]$Para, [int]$Number) {
    # "(番号)" を Meiryo UI / 薄い青 / イタリック / 14pt で先頭に挿入（スペースなし）
...
# 段落が画像を含むか
function Test-HasImage([System.Xml.XmlNode]$Para) {
    $NS_WP = "http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing"
    $NS_W2 = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
    return (
        $Para.GetElementsByTagName("inline", $NS_WP).Count -gt 0 -or
        $Para.GetElementsByTagName("anchor", $NS_WP).Count -gt 0 -or
        $Para.GetElementsByTagName("pict",   $NS_W2).Count -gt 0
    )
}

# 段落が改ページのみか（テキストなし・画像なし）
function Test-IsPageBreakOnly([System.Xml.XmlNode]$Para) {
    $text = (Get-ParaText $Para).Trim()
    if ($text -ne "") { return $false }
    if (Test-HasImage $Para) { return $false }
    foreach ($br in $Para.GetElementsByTagName("br", $NS_W)) {
        if ($br.GetAttribute("type", $NS_W) -eq "page") { return $true }
    }
    return $false
}

# 段落が空か（テキストなし・改ページなし・画像なし）
function Test-IsEmptyPara([System.Xml.XmlNode]$Para) {
    $text = (Get-ParaText $Para).Trim()
    if ($text -ne "") { return $false }
    if (Test-HasImage $Para) { return $false }
    foreach ($br in $Para.GetElementsByTagName("br", $NS_W)) {
        if ($br.GetAttribute("type", $NS_W) -eq "page") { return $false }
    }
    return $true
}

# 段落内の画像run・改ページrunを除去する（テキストは保持）
function Remove-ImageAndPageBreakRuns([System.Xml.XmlNode]$Para) {
    $toRemove = @()
    foreach ($r in $Para.ChildNodes) {
        if ($r.LocalName -ne "r") { continue }
        $NS_WP = "http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing"
        $NS_W2 = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
        $hasImg = (
            $r.GetElementsByTagName("inline", $NS_WP).Count -gt 0 -or
            $r.GetElementsByTagName("anchor", $NS_WP).Count -gt 0 -or
            $r.GetElementsByTagName("pict",   $NS_W2).Count -gt 0
        )
        $hasPageBr = $false
        foreach ($br in $r.GetElementsByTagName("br", $NS_W)) {
            if ($br.GetAttribute("type", $NS_W) -eq "page") { $hasPageBr = $true; break }
        }
        # テキストを持たない run のみ除去
        $hasText = ($r.GetElementsByTagName("t", $NS_W).Count -gt 0 -and (Get-ParaText $r).Trim() -ne "")
        if (($hasImg -or $hasPageBr) -and -not $hasText) { $toRemove += $r }
    }
    foreach ($n in $toRemove) { [void]$Para.RemoveChild($n) }
    return $toRemove.Count
}

# 画像を含む段落を処理する：
#   テキストなし → 段落ごと除去
#   テキストあり → 段落内の画像run・改ページrunのみ除去
function Remove-ImageParagraphs([System.Xml.XmlNode]$Body) {
    $children = @($Body.ChildNodes)
    ...
    for ($i = 0; $i -lt $children.Count; $i++) {
        $node = $children[$i]
        if ($node.LocalName -ne "p") { continue }
        if (-not (Test-HasImage $node)) { continue }
        ...
        if ($text -eq "") {
            ...
            # 直前の改ページ段落
            if ($i -gt 0 -and (Test-IsPageBreakOnly $children[$i-1])) {
                [void]$removeSet.Add($i-1)
            }
            # 直後の連続空段落・改ページ段落
            $j = $i + 1
            while ($j -lt $children.Count -and $children[$j].LocalName -eq "p" -and
                   (Test-IsEmptyPara $children[$j] -or Test-IsPageBreakOnly $children[$j])) {
                [void]$removeSet.Add($j); $j++
            }
        } else {
            # テキストあり → run内の画像・改ページだけ除去
            $n = Remove-ImageAndPageBreakRuns -Para $node
            $inlineCount += $n
        }
    }

    $count = 0
    foreach ($idx in ($removeSet | Sort-Object -Descending)) {
        [void]$Body.RemoveChild($children[$idx])
        $count++
    }
    $total = $count + $inlineCount
    if ($total -gt 0) {
        Write-Host ("     -> " + $count + " image paragraph(s) removed, " + $inlineCount + " inline image run(s) stripped") -ForegroundColor DarkGray
    }
    return $total
}
...
function Save-SectionDocXml(
    ...
    # 画像段落を除去
    $removedCount = Remove-ImageParagraphs -Body $body
    if ($removedCount -gt 0) {
        Write-Host ("     -> " + $removedCount + " image paragraph(s) removed") -ForegroundColor DarkGray
        # firstParaNode が除去された場合は再取得
        $firstParaNode = $null
        foreach ($c in $body.ChildNodes) {
            if ($c.LocalName -eq "p") { $firstParaNode = $c; break }
        }
    }
...
# 空白ページ除去スクリプトをtmpに展開
$pyScript = Join-Path $tmpRoot "remove_blank_pages.py"
$pyLines = @(
    "import sys, re, shutil, pypdf",
    "",
    "def normalize_text(page):",
    ...
    "    return False",
    "",
    "def remove_blank_pages(src, dst):",
    ...
    "    n = remove_blank_pages(src, dst)",
    "    print(n)"
)
[System.IO.File]::WriteAllLines($pyScript, $pyLines, [System.Text.UTF8Encoding]::new($false))
...
try {
    ...
    for ($si = 0; $si -lt $sections.Count; $si++) {
        ...
        if (-not (Convert-ToPdf -DocxPath $outDocx -PdfPath $outPdf)) {
            ...
            # 空白ページ除去
            $tmpPdf = $outPdf + ".tmp.pdf"
            try {
                # python/python3 どちらか使えるコマンドで実行
                $pyCmd = $null
                ...
                if ($pyCmd -eq $null) {
                    Write-Host "     (python not found, blank page removal skipped)" -ForegroundColor Yellow
                } else {
                    # pypdf インストール確認
                    $chkPypdf = & $pyCmd -c "import pypdf" 2>&1
                    if ($LASTEXITCODE -ne 0) {
                        Write-Host "     (installing pypdf...)" -ForegroundColor Yellow
                        & $pyCmd -m pip install pypdf --quiet 2>&1 | Out-Null
                    }
                    $removed = & $pyCmd $pyScript $outPdf $tmpPdf 2>&1
                    Write-Host ("     python exit=" + $LASTEXITCODE + " removed=" + ($removed -join " ")) -ForegroundColor DarkGray
                    if ($LASTEXITCODE -eq 0 -and ($removed -as [int]) -gt 0) {
                        Move-Item -LiteralPath $tmpPdf -Destination $outPdf -Force
                        Write-Host ("     -> " + $removed + " blank page(s) removed") -ForegroundColor DarkGray
                    } elseif (Test-Path -LiteralPath $tmpPdf) {
                        Remove-Item -LiteralPath $tmpPdf -Force
                    }
                }
            } catch {
                Write-Host ("     blank page removal error: " + $_.Exception.Message) -ForegroundColor Yellow
