...
# 空白ページ除去スクリプトをtmpに生成（BOMなしUTF-8）
...
# python/python3 コマンドを検出
$pyCmd = $null
foreach ($cmd in @("python","python3")) {
    try {
        $chk = & $cmd -c "import pypdf; print('ok')" 2>&1
        if ($chk -match "ok") { $pyCmd = $cmd; break }
    } catch {}
}
if ($pyCmd -eq $null) {
    # pypdfが未インストールの場合インストールを試みる
    foreach ($cmd in @("python","python3")) {
        try {
            & $cmd --version 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) {
                Write-Host "     (installing pypdf...)" -ForegroundColor Yellow
                & $cmd -m pip install pypdf --quiet 2>&1 | Out-Null
                $chk = & $cmd -c "import pypdf; print('ok')" 2>&1
                if ($chk -match "ok") { $pyCmd = $cmd; break }
            }
        } catch {}
...
if ($pyCmd) {
    Write-Host ("     blank page removal: using " + $pyCmd) -ForegroundColor DarkGray
} else {
    Write-Host "     blank page removal: python/pypdf not available, skipping" -ForegroundColor Yellow
}
        ...
        if (-not (Convert-ToPdf -DocxPath $outDocx -PdfPath $outPdf)) {
            ...
            if ($pyCmd) {
                ...
                try {
                    ...
                    if ($LASTEXITCODE -eq 0 -and (($removed -join "") -as [int]) -gt 0) {
                        Move-Item -LiteralPath $tmpPdf -Destination $outPdf -Force
                        Write-Host ("     -> " + ($removed -join "") + " blank page(s) removed") -ForegroundColor DarkGray
